...
class TimeRange extends React.Component {
  ...
  //   console.log(scaleTime().domain(timelineInterval).ticks(ticksNumber).map((t) => +t));
  //   return scaleTime().domain(timelineInterval).ticks(ticksNumber).map((t) => +t);
  // };

  getDateTicks = () => {
    const { timelineInterval, ticksNumber } = this.props;
    const startYear = timelineInterval[0].getFullYear();
    const endYear = timelineInterval[1].getFullYear();
    const years = [];
    const range = endYear - startYear;
    const step = Math.floor(range / (ticksNumber - 1));

    for (let i = 0; i < ticksNumber - 1; i++) {
      years.push(new Date(startYear + (i * step), 0, 1).getTime());
    }
    years.push(new Date(endYear, 0, 1).getTime()); 
